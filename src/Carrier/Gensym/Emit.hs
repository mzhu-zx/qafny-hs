...
newtype GensymC s m a = GensymC { runGensymC :: StateC (Int, [s]) m a }
  deriving (Applicative, Functor, Monad)

instance (Variable s, Algebra sig m) => Algebra (Gensym s :+: sig) (GensymC s m) where
  alg hdl sig ctx = GensymC $ case sig of
    L (Gensym s) -> state $ \(i, w) -> ((i + 1 :: Int, s : w), variable (s, i) <$ ctx)
    R other      -> alg (runGensymC . hdl) (R other) ctx

runGensymEmit :: Functor m => GensymC s m a -> m (Int, [s], a)
runGensymEmit c = do
